...
st.title("3元連立1次方程式を掃き出し法で解く")

st.write("連立方程式")
# st.write("以下の形式で入力してください:")
st.write("a₁x + b₁y + c₁z = d₁")
st.write("a₂x + b₂y + c₂z = d₂")
st.write("a₃x + b₃y + c₃z = d₃")

st.write("### 拡大係数行列を入力（3行、各行を空白で区切る）:")
...
def matrix_to_latex(matrix):
    ...
    latex_str = r"\left(\begin{array}{cccc}" + "\n"
    latex_str += " \\\\ ".join(rows)
...
try:
    ...
    if len(matrix) != 3 or any(len(row) != 4 for row in matrix):
        st.error("エラー: 3×4 の拡大係数行列が必要です。")
    else:
        # セッション状態の初期化
        if 'A' not in st.session_state:
            st.session_state.A = np.array(matrix, dtype=float)
            st.session_state.history = []
        
        # 初期状態を履歴に追加
        if len(st.session_state.history) == 0:
            st.session_state.history.append(("初期状態", st.session_state.A.copy()))
        ...
        if operation == "ある行に他の行の何倍か加える":
            ...
            if st.button("実行"):
                ...
                if k is None:
                    ...
                    st.session_state.A[target_row] = st.session_state.A[target_row] + k * st.session_state.A[source_row]
                    description = f"R{target_row + 1} ← R{target_row + 1} + {Fraction(k).limit_denominator(100)} × R{source_row + 1}"
                    st.session_state.history.append((description, st.session_state.A.copy()))
                    st.rerun()
            ...
            if st.button("実行"):
                ...
                if k is None:
                    ...
                    st.session_state.A[target_row] = k * st.session_state.A[target_row]
            ...
            if st.button("実行"):
                st.session_state.A[[row1, row2]] = st.session_state.A[[row2, row1]]
        ...
        with col1:
            if st.button("直前の操作を取り消す（Undo）"):
                if len(st.session_state.history) > 1:
                    ...
                    st.session_state.A = st.session_state.history[-1][1].copy()
                    st.rerun()
                else:
        ...
        with col2:
            if st.button("全てリセット"):
                st.session_state.A = np.array(matrix, dtype=float)
                st.session_state.history = [("初期状態", st.session_state.A.copy())]
                st.rerun()
        ...
        st.write("---")
        st.write("## 解の確認")
        if st.button("現在の行列から解を抽出"):
            try:
                x = st.session_state.A[0, -1]
                y = st.session_state.A[1, -1]
                z = st.session_state.A[2, -1]
                ...
                st.write("### 解")
                st.latex(f"x = {x_frac}, \\quad y = {y_frac}, \\quad z = {z_frac}")
